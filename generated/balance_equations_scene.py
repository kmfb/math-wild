# Auto-generated by compile_to_manim.py. Do not edit by hand.
from manim import *
...
ONE_LINER = '解方程不是搬家，而是在不破坏平衡的前提下，让隐藏量显现。'

def make_header(title=TITLE, subtitle=SUBTITLE):
    return chapter_header(title, subtitle)

def make_main_balance():
    return balance_from_items([{'kind': 'unknown', 'label': 'x', 'count': 1}, {'kind': 'unit', 'count': 3}], [{'kind': 'unit', 'count': 8}], scale=1.05)

def make_after_balance():
    return balance_from_items([{'kind': 'unknown', 'label': 'x', 'count': 1}], [{'kind': 'unit', 'count': 5}], scale=1.05)

def make_two_x_balance():
    return balance_from_items([{'kind': 'unknown', 'label': 'x', 'count': 2}], [{'kind': 'unit', 'count': 10}], scale=1.02)

class GeneratedBalanceEquations(Scene):
    def construct(self):
        header = make_header()
