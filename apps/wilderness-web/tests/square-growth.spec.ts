...
test("user can grow odd rings into square structure", async ({ page }) => {
  await openHero(page);

  const grow = page.getByRole("button", { name: "Grow one ring" });
  await grow.click();
...
test("formula and hundred square climax are reachable", async ({ page }) => {
  await openHero(page);
